...
impl HttpConfig {
    pub fn from_env() -> Result<Self> {
        let host =
            utils::envs::get_env("HOST").map_err(|_| Error::MissingEnv { key: "HOST" })?;
        let port_str =
            utils::envs::get_env("PORT").map_err(|_| Error::MissingEnv { key: "PORT" })?;

        let session_secret =
            utils::envs::get_env_b64u_as_u8s("SESSION_SECRET").map_err(|_| {
                Error::InvalidEnv {
                    key: "SESSION_SECRET",
                    reason: EnvErrorReason::new("must be base64url without padding"),
                }
            })?;
        ...
        let session_cleanup_interval_secs =
            match std::env::var("SESSION_CLEANUP_INTERVAL_SECS") {
                Ok(value) => value.parse().map_err(|_| Error::InvalidEnv {
                    ...
                    reason: EnvErrorReason::new("must be a valid u64 integer"),
                })?,
        ...
        Ok(Self {
            ...
            port: port_str.parse().map_err(|_| Error::InvalidEnv {
                key: "PORT",
                reason: EnvErrorReason::new("must be a valid u16 integer"),
            })?,
            session_secret,
