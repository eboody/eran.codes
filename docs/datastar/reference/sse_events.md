...
Responses to [backend actions](actions.md#backend-actions) with a content type of `text/event-stream` can contain zero or more Datastar [SSE events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events).

> The backend [SDKs](sdks.md) can handle the formatting of SSE events for you, or you can format them yourself.
