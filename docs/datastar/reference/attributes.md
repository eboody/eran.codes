...
> If you want files to be uploaded to the server, rather than be converted to signals, use a form and with `multipart/form-data` in the [`enctype`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/enctype) attribute. See the [backend actions](actions.md#backend-actions) reference.
...
Signals beginning with an underscore are _not_ included in requests to the backend by default. You can opt to include them by modifying the value of the [`filterSignals`](actions.md#filterSignals) option.
...
### `data-animate` [Pro](/datastar_pro "Datastar Pro")
...
### `data-custom-validity` [Pro](/datastar_pro "Datastar Pro")
...
### `data-on-raf` [Pro](/datastar_pro "Datastar Pro")
...
### `data-on-resize` [Pro](/datastar_pro "Datastar Pro")
...
### `data-persist` [Pro](/datastar_pro "Datastar Pro")
...
### `data-query-string` [Pro](/datastar_pro "Datastar Pro")
...
### `data-replace-url` [Pro](/datastar_pro "Datastar Pro")
...
### `data-scroll-into-view` [Pro](/datastar_pro "Datastar Pro")
...
### `data-rocket` [Pro](/datastar_pro "Datastar Pro")

Creates a Rocket web component. See the [Rocket reference](rocket.md) for details.

### `data-view-transition` [Pro](/datastar_pro "Datastar Pro")
...
Data attributes are evaluated and applied on page load (after Datastar has initialized), and are reapplied after any DOM patches that add, remove, or change them. Note that [morphing elements](sse_events.md#datastar-patch-elements) preserves existing attributes unless they are explicitly changed in the DOM, meaning they will only be reapplied if the attribute itself is changed.
...
It is possible to alias `data-*` attributes to a custom alias (`data-alias-*`, for example) using the [bundler](/bundler). A custom alias should _only_ be used if you have a conflict with a legacy library and `data-ignore` cannot be used.
...
Read more about [Datastar expressions](../guide/datastar_expressions.md) in the guide.
...
The “More info” link takes you directly to a context-aware error page that explains the error and provides correct sample usage. See [the error page for the example above](/errors/key_not_allowed?metadata=%7B%22plugin%22%3A%7B%22name%22%3A%22text%22%2C%22type%22%3A%22attribute%22%7D%2C%22element%22%3A%7B%22id%22%3A%22%22%2C%22tag%22%3A%22DIV%22%7D%2C%22expression%22%3A%7B%22rawKey%22%3A%22textFoo%22%2C%22key%22%3A%22foo%22%2C%22value%22%3A%22%22%2C%22fnContent%22%3A%22%22%7D%7D), and all available error messages in the sidebar menu.
