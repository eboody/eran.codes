...
Sends a `GET` request to the backend using the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API). The URI can be any valid endpoint and the response must contain zero or more [Datastar SSE events](sse_events.md).
...
Backend actions automatically handle different response content types:
  ...
  * `text/event-stream` – Standard SSE responses with [Datastar SSE events](sse_events.md).
...
### `@clipboard()` [Pro](/datastar_pro "Datastar Pro")
...
### `@fit()` [Pro](/datastar_pro "Datastar Pro")
