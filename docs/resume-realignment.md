...
- background token refresh and bounded local HTTP-backed record sync
...
- live third-party vendor connectivity
- real PHI handling
...
- The secure-data slice now proves a local HTTP integration boundary as well as mechanics and boundary discipline, but not live third-party operations.
...
- a separate stronger runtime-proof branch if the resume needs deeper authorization or real external integration evidence
