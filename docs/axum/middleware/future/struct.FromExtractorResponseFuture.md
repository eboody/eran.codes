<!-- Generated from rustdoc HTML: middleware/future/struct.FromExtractorResponseFuture.html -->
...
# Struct FromExtractorResponseFuture Copy item path
...
Response future for [`FromExtractor`][7].

## Trait Implementations§

[Source][8]§

### impl<B, T, E, S> [Future][9] for [ResponseFuture][10]<B, T, E, S>

where E: [FromRequestParts][6]<S>, T: Service<Request<B>>, T::Response: [IntoResponse][11],

[Source][12]§

#### type [Output][13] = [Result][14]<Response<[Body][15]>, <T as Service<Request<B>>>::Error>

The type of value produced on completion.

[Source][16]§

#### fn [poll][17](self: [Pin][18]<&mut Self>, cx: &mut [Context][19]<'_>) -> [Poll][20]<Self::[Output][21]>

Attempts to resolve the future to a final value, registering the current task for wakeup if the value is not yet available. [Read more][17]

[Source][5]§

### impl<'__pin, B, T, E, S> [Unpin][22] for [ResponseFuture][10]<B, T, E, S>

where PinnedFieldsOf<__Origin<'__pin, B, T, E, S>>: [Unpin][22], E: [FromRequestParts][6]<S>, T: Service<Request<B>>,

## Auto Trait Implementations§

§

### impl<B, T, E, S> [Freeze][23] for [ResponseFuture][10]<B, T, E, S>

where <T as Service<Request<B>>>::Future: [Freeze][23], T: [Freeze][23],

§

### impl<B, T, E, S> ![RefUnwindSafe][24] for [ResponseFuture][10]<B, T, E, S>

§

### impl<B, T, E, S> [Send][25] for [ResponseFuture][10]<B, T, E, S>

where <T as Service<Request<B>>>::Future: [Send][25], T: [Send][25],

§

### impl<B, T, E, S> ![Sync][26] for [ResponseFuture][10]<B, T, E, S>

§

### impl<B, T, E, S> ![UnwindSafe][27] for [ResponseFuture][10]<B, T, E, S>
...
where T: [Future][9] \+ ?[Sized][30],

§

#### fn map<U, F>(self, f: F) -> Map<Self, F>

where F: [FnOnce][47](Self::[Output][21]) -> U, Self: [Sized][30],
...
where Self::[Output][21]: [Into][48]<U>, Self: [Sized][30],
...
where F: [FnOnce][47](Self::[Output][21]) -> Fut, Fut: [Future][9], Self: [Sized][30],
...
where B: [Future][9]<Output = Self::[Output][21]>, Self: [Sized][30],
...
where A: [Future][9]<Output = Self::[Output][21]>, Self: [Sized][30],
...
where Self::[Output][21]: [Future][9], Self: [Sized][30],
...
where Self::[Output][21]: Stream, Self: [Sized][30],
...
where F: [FnOnce][47](&Self::[Output][21]), Self: [Sized][30],
...
where Self: [Sized][30], Self::[Output][21]: [Clone][49],
...
#### fn boxed<'a>(self) -> [Pin][18]<[Box][50]<dyn [Future][9]<Output = Self::[Output][21]> \+ [Send][25] \+ 'a>>
...
#### fn boxed_local<'a>(self) -> [Pin][18]<[Box][50]<dyn [Future][9]<Output = Self::[Output][21]> \+ 'a>>
...
Turns a [`Future<Output = T>`][9] into a [`TryFuture<Ok = T, Error = ()`>][51].
...
Turns a [`Future<Output = T>`][9] into a [`TryFuture<Ok = T, Error = Never`>][51].

§

#### fn poll_unpin(&mut self, cx: &mut [Context][19]<'_>) -> [Poll][20]<Self::[Output][21]>
...
#### fn now_or_never(self) -> [Option][52]<Self::[Output][21]>
...
#### type [Output][61] = <F as [Future][9]>::[Output][21]
...
#### fn [try_from][82](value: U) -> [Result][14]<T, <T as [TryFrom][77]<U>>::[Error][83]>
...
where F: [Future][9]<Output = [Result][14]<T, E>> \+ ?[Sized][30],
...
#### fn try_poll( self: [Pin][18]<[&mut F][38]>, cx: &mut [Context][19]<'_>, ) -> [Poll][20]<<F as [Future][9]>::[Output][21]>
...
Maps this future’s [`Error`][84] to a new error type using the [`Into`][48] trait. Read more
...
Maps this future’s [`Ok`][85] to a new type using the [`Into`][48] trait.
...
Unwraps this future’s output, producing a future with this future’s [`Ok`][85] type as its [`Output`][86] type. Read more
...
Wraps a [`TryFuture`] into a type that implements [`Future`][9]. Read more

§

#### fn try_poll_unpin( &mut self, cx: &mut [Context][19]<'_>, ) -> [Poll][20]<[Result][14]<Self::Ok, Self::Error>>
...
#### type [Error][90] = <U as [TryFrom][77]<T>>::[Error][83]

The type returned in the event of a conversion error.

[Source][91]§

#### fn [try_into][92](self) -> [Result][14]<U, <U as [TryFrom][77]<T>>::[Error][83]>
...
Attaches the current [default][94] [`Subscriber`][93] to this type, returning a [`WithDispatch`] wrapper. Read more
   ...
   [3]: ../../index.html
   [4]: ../index.html
   [5]: ../../../src/axum/middleware/from_extractor.rs.html#234-246
   [6]: ../../extract/trait.FromRequestParts.html (trait axum::extract::FromRequestParts)
   [7]: ../struct.FromExtractor.html (struct axum::middleware::FromExtractor)
   [8]: ../../../src/axum/middleware/from_extractor.rs.html#262-300
   [9]: https://doc.rust-lang.org/nightly/core/future/future/trait.Future.html (trait core::future::future::Future)
   [10]: struct.FromExtractorResponseFuture.html (struct axum::middleware::future::FromExtractorResponseFuture)
   [11]: ../../response/trait.IntoResponse.html (trait axum::response::IntoResponse)
   [12]: ../../../src/axum/middleware/from_extractor.rs.html#268
   [13]: https://doc.rust-lang.org/nightly/core/future/future/trait.Future.html#associatedtype.Output
   [14]: https://doc.rust-lang.org/nightly/core/result/enum.Result.html (enum core::result::Result)
   [15]: ../../body/struct.Body.html (struct axum::body::Body)
   [16]: ../../../src/axum/middleware/from_extractor.rs.html#270-299
   [17]: https://doc.rust-lang.org/nightly/core/future/future/trait.Future.html#tymethod.poll
   [18]: https://doc.rust-lang.org/nightly/core/pin/struct.Pin.html (struct core::pin::Pin)
   [19]: https://doc.rust-lang.org/nightly/core/task/wake/struct.Context.html (struct core::task::wake::Context)
   [20]: https://doc.rust-lang.org/nightly/core/task/poll/enum.Poll.html (enum core::task::poll::Poll)
   [21]: https://doc.rust-lang.org/nightly/core/future/future/trait.Future.html#associatedtype.Output (type core::future::future::Future::Output)
   ...
   [84]: TryFuture::Error
   [85]: TryFuture::Ok
   [86]: https://doc.rust-lang.org/nightly/core/future/future/trait.Future.html#associatedtype.Output (associated type core::future::future::Future::Output)
