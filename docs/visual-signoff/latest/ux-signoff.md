...
- Full-site UX signoff is still blocked by the open-source route, which still mismatches the committed desktop-light baselines in both guest and signed-in states.
- The signed-in `/` route renders the portfolio home flow; earlier capture failures were assertion drift, not a missing route-state problem.
