...
pub async fn auth_status_partial(
    ...
    let prepared = auth_status_flow::IncomingFlow::new().prepare_snapshot(
        &auth_session,
        &session,
        &state,
    );
...
pub async fn session_status_partial(
    ...
    let prepared =
        session_status_flow::IncomingFlow::new().prepare_snapshot(&session, &state);
    prepared.into_response()
}
...
pub async fn request_meta_partial(
    ...
    let prepared = request_meta_flow::IncomingFlow::new().prepare_snapshot(&state);
    prepared.into_response()
}
...
pub async fn boundary_check_partial(
    ...
    let incoming = boundary_check_flow::IncomingFlow::from_query(query.case);
...
pub async fn db_check_partial(
    ...
    let incoming = db_check_flow::IncomingFlow::from_query(query.email);
