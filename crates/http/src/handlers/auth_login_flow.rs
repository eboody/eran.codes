...
impl LoginFlow<Incoming> {
    pub(super) fn from_form(form: LoginForm, next: Option<String>) -> crate::Result<Self> {
...
mod tests {
    ...
    fn apply_auth_result_routes_to_authenticated_state() {
        let incoming =
            LoginFlow::<Incoming>::from_form(login_form(), Some("/protected".to_string()))
                .expect("incoming");
