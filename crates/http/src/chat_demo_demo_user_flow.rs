...
impl DemoUserEnsureFlow<Incoming> {
    pub(super) fn prepare_identity(
        ...
        let email = domain::user::Email::try_new(email_literal)
            .map_err(|_| crate::Error::Internal)?;
        let username = domain::user::Username::try_new(username_literal)
            .map_err(|_| crate::Error::Internal)?;
...
mod tests {
    ...
    fn prepare_identity_rejects_invalid_email() {
        let incoming = DemoUserEnsureFlow::<Incoming>::new();
