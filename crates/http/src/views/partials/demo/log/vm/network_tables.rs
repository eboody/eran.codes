...
pub fn request_rows(entries: &[store::TraceEntry]) -> Vec<Vec<Markup>> {
    entries
        .iter()
        .filter(|entry| {
            matches!(entry.target_kind(), target::Kind::Known(target::Known::DemoRequest))
                && matches!(
                    entry.message_kind(),
...
pub fn sse_rows(entries: &[store::TraceEntry]) -> Vec<Vec<Markup>> {
    entries
        .iter()
        .filter(|entry| matches!(entry.target_kind(), target::Kind::Known(target::Known::DemoSse)))
        .rev()
        .take(20)
        .map(|entry| {
            vec![
                ...
                maud::html! { (entry.field_text(LogFieldKey::Selector).cloned().unwrap_or_else(|| Text::from("-"))) },
                maud::html! { (entry.field_text(LogFieldKey::Mode).cloned().unwrap_or_else(|| Text::from("-"))) },
                maud::html! { (entry.field_text(LogFieldKey::PayloadBytes).cloned().unwrap_or_else(|| Text::from("-"))) },
            ]
        })
        .collect()
}
...
fn method_pill(entry: &store::TraceEntry) -> Markup {
    match entry.field_text(LogFieldKey::Method) {
        Some(method) => components::Pill::method(method.clone()).render(),
        None => components::Pill::fields("-").render(),
    }
}

fn path_pill(entry: &store::TraceEntry) -> Markup {
    match entry.field_text(LogFieldKey::Path) {
        Some(path) => components::Pill::path(path.clone()).render(),
        None => components::Pill::fields("-").render(),
    }
}

fn status_pill(entry: &store::TraceEntry) -> Markup {
    match entry.field_text(LogFieldKey::Status) {
        Some(status) => components::Pill::status(status.clone()).render(),
        None => components::Pill::fields("-").render(),
    }
}
