...
3. Audit layout hierarchy:
   ...
   - copy density and label length for component readability
...
8. Validate interaction intelligibility with fresh visual evidence:
   - click each relevant control state (not just default page load)
   - confirm active/inactive state is perceivable in screenshots
   - if visuals intentionally changed, refresh baseline in the same pass
...
- When matching a reference product, adjust microcopy length/tone so visual structure reads like the reference (not just colors/borders).
...
- If visual output intentionally changes, refresh and report visual baseline status (`VISUAL_UPDATE_BASELINE=1`).
- Visual QA is incomplete without fresh interactive screenshots (for tabs/carousels/etc., capture each reviewed state).
