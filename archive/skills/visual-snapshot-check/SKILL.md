...
4. Run the visual check:
   ...
   - the script now prunes `artifacts/visual/current/*.png` before capture to prevent stale screenshot reuse.
   - override prune pattern with `VISUAL_PRUNE_PATTERN='*.webp'` (or any glob) only when needed.
   - set `VISUAL_PRUNE_CURRENT=0` only when you intentionally need to preserve prior captures.
5. If check fails, inspect:
   ...
   - tab states: `artifacts/visual/current/showcase_tab_0.png` ... `showcase_tab_3.png`
   - for NetBird side-by-side reference capture: `scripts/ci/visual-reference-capture.sh`
...
7. Record UX/UI signoff artifacts for visual commits:
   ...
   - `artifacts/visual/audits/latest/rust-systems-hiring-manager-signoff.md`
   - `artifacts/visual/audits/latest/signoff.env`
   - include `REVIEW_SCOPE`, `REVIEWED_COMMIT_SHA`, `REVIEWED_TREE_SHA`,
     `SNAPSHOT_HOME`, `SNAPSHOT_SHOWCASE_TAB_0..3`
...
- The signoff gate must fail on stale review metadata, missing interactive tab-state snapshots,
  missing UX/UI/hiring-manager artifacts, or any reviewer marked non-pass/unavailable.
